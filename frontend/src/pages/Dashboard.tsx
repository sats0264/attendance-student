  ...
  const renderAdminDashboard = () => (
    <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-8">
           <div className="flex flex-col gap-6">
             <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.1 }} className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-400 w-fit backdrop-blur-md">
               ...
               <span className="text-xs font-black tracking-widest uppercase">Espace Administrateur</span>
             </motion.div>
             <h1 className="text-5xl md:text-7xl font-black text-white tracking-tighter drop-shadow-2xl">
               Centre de <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 via-indigo-400 to-purple-400">Contrôle</span>
             </h1>
             <p className="text-lg md:text-xl text-white/60 max-w-2xl leading-relaxed font-light">
               Supervisez l'écosystème <strong className="text-white/90">AttendancePro</strong>. Gérez les effectifs, orchestrez les classes et analysez les flux de présence en temps réel avec une précision chirurgicale.
             </p>
           </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <StatCard title="Étudiants Inscrits" value={stats.totalStudents} icon={Users} color="text-blue-400" gradient="from-blue-600 to-indigo-900" delay={0.2} onClick={() => navigate('/students')} />
        <StatCard title="Classes Actives" value={stats.totalClasses} icon={BookOpen} color="text-purple-400" gradient="from-purple-600 to-fuchsia-900" delay={0.3} onClick={() => navigate('/classes')} />
        <StatCard title="Corps Professoral" value={stats.totalTeachers} icon={GraduationCap} color="text-emerald-400" gradient="from-emerald-600 to-teal-900" delay={0.4} onClick={() => navigate('/admin/teachers')} />
        <StatCard title="Séances Enregistrées" value={stats.totalSessions} icon={Calendar} color="text-orange-400" gradient="from-orange-600 to-red-900" delay={0.5} />
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
         ...
         <div className="lg:col-span-1 flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-4">
              <QuickActionCard title="Enrôlement Biométrique" description="Ajouter un étudiant avec reconnaissance faciale" icon={Camera} onClick={() => navigate('/enrollment')} delay={0.6} colorClass="from-blue-600 to-indigo-600" />
              <QuickActionCard title="Gestion des Enseignants" description="Configurer les profils et accès académiques" icon={ShieldCheck} onClick={() => navigate('/admin/teachers')} delay={0.7} colorClass="from-emerald-600 to-teal-600" />
              <QuickActionCard title="Structure Académique" description="Organiser les classes et promotions" icon={BookOpen} onClick={() => navigate('/classes')} delay={0.8} colorClass="from-orange-600 to-rose-600" />
            </div>
         </div>
  ...
  const renderTeacherDashboard = () => (
    <div className="flex flex-col gap-8 w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <div className="relative z-10 flex flex-col xl:flex-row items-center justify-between gap-12">
           <div className="flex flex-col gap-6 max-w-2xl">
             <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="inline-flex items-center gap-2 px-5 py-2 rounded-full bg-white/5 border border-white/10 text-white/80 w-fit backdrop-blur-md">
               ...
               <span className="text-xs font-black tracking-widest uppercase text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-pink-400">Espace Enseignant</span>
             </motion.div>
             <h1 className="text-5xl md:text-6xl lg:text-7xl font-black text-white tracking-tighter drop-shadow-2xl leading-[1.1]">
               Bonjour, <br/>
               <span className="text-transparent bg-clip-text bg-gradient-to-r from-purple-400 via-pink-400 to-rose-400">{userData?.name || 'Professeur'}</span>
             </h1>
             <p className="text-lg md:text-xl text-white/60 leading-relaxed font-light">
               Transformez l'appel d'un geste. Démarrez l'enregistrement des présences grâce à <strong className="text-white/90">l'IA visuelle</strong> et concentrez-vous sur l'essentiel : enseigner.
             </p>
           </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <StatCard title="Mes Étudiants" value={stats.totalStudents} icon={Users} color="text-purple-400" gradient="from-purple-600 to-fuchsia-900" delay={0.2} />
        <StatCard title="Séances Validées" value={stats.totalSessions} icon={CheckCircle} color="text-pink-400" gradient="from-pink-600 to-rose-900" delay={0.3} />
        <StatCard title="Classes Assignées" value={teacherClasses.length} icon={BookOpen} color="text-indigo-400" gradient="from-indigo-600 to-blue-900" delay={0.4} onClick={() => navigate('/classes')} />
      </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              {teacherClasses.length === 0 ? (
                 <div className="glass-panel p-12 rounded-[2rem] text-center border border-white/5 opacity-80">
                   ...
                   <p className="text-white/50 text-lg">Aucune classe assignée pour le moment.</p>
                 </div>
              ) : (
  ...
  if (recentActivity.length === 0) {
    return (
      <div className="p-16 text-center opacity-70 flex flex-col items-center gap-6 h-full justify-center">
        ...
        <p className="text-2xl font-black tracking-tight">Aucune donnée récente</p>
        <p className="text-base text-white/50 max-w-sm mx-auto font-medium">Les enregistrements de présence apparaîtront automatiquement ici après l'évaluation d'une séance.</p>
      </div>
    );
  }
